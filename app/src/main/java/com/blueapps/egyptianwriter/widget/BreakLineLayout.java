package com.blueapps.egyptianwriter.widget;

// Inlined from https://github.com/zerozhiqin/AutoBreakLineLayout (MIT License)
// Aggiornato: android.support.v4.view.ViewCompat -> androidx.core.view.ViewCompat
...
public class BreakLineLayout extends ViewGroup {
    ...
    public void setShowLines(int showLines) {
        if (this.showLines == showLines) return;
    ...
    public void setShowAll() {
        if (this.showLines == Integer.MAX_VALUE) return;
        oldShowLines = this.showLines;
        ...
        while (index < childCount) {
            ...
            if (currentW + childW < viewW) {
                ...
                if (currentW == 0) {
                    ...
                    line.width = currentW;
                    lines.add(line);
                    if (lines.size() >= showLines) { breakLine = true; break; }
                    currentW = 0;
                    line = new Line();
                }
            }
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        for (Line line : lines) {
            ...
            for (View childView : line.childs) {
                ...
                LayoutParams childLp = (LayoutParams) childView.getLayoutParams();
                childView.layout(
                        left + childLp.leftMargin + widthAdd,
                ...
                if (index > oldShowLines - 1) {
                    ViewCompat.setAlpha(childView, 0f);
                    ViewCompat.animate(childView).alpha(1f).setDuration(300).start();
                }
            }
            index++;
            left = getPaddingLeft();
