 ...
 * here is guessed. This verification predates the audit-EG (2026-07-22)
 * change that made [BlissLookup.nameOf] language-aware
 * (`bci_names_<lang>.json`, generated by `tools/bci_names_split.py`);
 * `bci_names.json` itself is unaffected (still bundled, now used only as a
 * defensive fallback) and every id↔name fact verified against it below still
 * holds, since it's still a valid subset of the same BCI-AV id space. Six
 * official indicators are deliberately **not**
